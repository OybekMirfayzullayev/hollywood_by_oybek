...
export default function Actorinfo() {
  ...
  const getInformation = () => {
    axios
      .get(
        `https://6766a280410f849996582c47.mockapi.io/actors/${params?.actor_Id} `
      )
      .then((res) => {
        setActor(res?.data);
      })
  ...
  const getFilms = () => {
    axios
      .get(
        `https://6766a280410f849996582c47.mockapi.io/actors/${params?.actor_Id}/films`
      )
      .then((res) => {
        setFilm(res?.data);
      })
          ...
          {actor_info && (
            <div className="flex flex-wrap w-full mb-10">
              <div className="lg:w-1/2 w-full lg:mb-0 text-center">
                <h1 className="sm:text-3xl text-2xl font-medium title-font text-gray-900 mt-4 dark:text-white">
                  {actor_info.full_name}
                </h1>
              </div>
              <p className="lg:w-1/2 w-full leading-relaxed text-gray-500 dark:text-white">
                {actor_info.information}
              </p>
            </div>
          )}
                ...
                <div className="xl:w-1/4 md:w-1/2 p-3" key={item?.id}>
                  <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-700">
                    <img
                      ...
                      alt="content"
                    />
                    <h3 className="tracking-widest text-indigo-500 text- font-medium title-font dark:text-white">
                      {item?.movie_year}
                    </h3>
